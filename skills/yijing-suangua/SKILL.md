...
description: 易经占卜大师——调用李小问 API 起卦、占卜、解卦。支持两种渲染风格：「图文」（默认，适配网页、小程序、手机 App、客户端）直接透传 API 返回的 Markdown；「艺术字」（适配 Claude Code、终端、命令行）额外包装 ASCII 卦象图 + 可点击图片链接 + ASCII 评级框。当用户提到"占卜、算卦、起卦、摇卦、解卦、易经、周易、六爻、卦象、问卦、求签、测一卦"等任何易经占卜相关需求时务必触发；即使用户只是说"帮我算一下""给我卜一卦""这事能不能成"等含糊表达，只要上下文指向求卦问事，也应使用本 skill。
...
║                    !!  首 次 使 用 必 读  !!                         ║
║                                                                      ║
║   本 skill 需要配置两项，否则起卦会失败或渲染不好看：                ║
║                                                                      ║
║     ① API_KEY（必填）                                               ║
║        获取方式（二选一）：                                          ║
║          · 微信搜索小程序【李小问】→「我的」页面复制                 ║
║          · 浏览器打开 https://wenmutang.top → 微信扫码进入小程序     ║
║                                                                      ║
║     ② RENDER_STYLE（渲染风格，必填、默认「图文」）                            ║
║        · 图文    → 看图片 + 普通文字（推荐 99% 用户）               ║
║          适用：网页、小程序、App、QoderWork、悟空 等                 ║
║        · 艺术字  → 看 ASCII 字符画（仅限命令行用户）                 ║
║          适用：Claude Code 等终端cli模式                                     ║
║        不确定？保持默认「图文」即可                                  ║
║                                                                      ║
╚══════════════════════════════════════════════════════════════════════╝
```

> 配置完成后直接说"帮我起一卦""算一下这事能不能成"即可触发；若遇 401/403 鉴权错误，说明 API Key 未配置或无效。

## 配置

以下两个变量需要配置。**Agent 读取本节时**：

- 若值仍为 `<YOUR_...>` 占位符 → 说明用户未配置，需引导用户配置
- 若值已填 → 说明已配置，直接使用
...
- **API_KEY**：`<YOUR_API_KEY>`
  - **必填**，否则起卦会报 401 鉴权错误
  - 未配置时：引导用户获取 Key → 本 agent 有权限则写入本文件，无权限则引导用户手动编辑本文件
  - 获取方式（二选一）：
    - 微信搜索小程序【李小问】→「我的」页面复制
...
- **RENDER_STYLE**：`<YOUR_RENDER_STYLE>`
  - **必填**，否则默认按图文渲染
  - 未配置时：说明两种风格差异让用户确认 → 本 agent 有权限则写入本文件，无权限则引导用户手动编辑本文件
  - 可选值：
...
### 分支 A：`RENDER_STYLE = 图文`（默认，适配网页/小程序/手机 App/客户端）
...
以下 ASCII 模板**仅在 `RENDER_MODE = cli` 时使用**，markdown 模式请完全忽略。
